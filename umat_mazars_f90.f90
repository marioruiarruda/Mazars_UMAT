...
!     |     ====           ==========           =====================  |
!     |  03/06/2025      MARIO RUI ARRUDA           STABILIZATION      |
!     |  LNEC LISBON                                 BIAXIAL-COEF      |
!     ------------------------------------------------------------------
...
! This means that, by default, any variables with first letter between a-h or o-z are real double.
...
call calc_macaulay_bracket(3,psije,psijebp,ONE)
call calc_macaulay_bracket(3,psije,psijebn,mONE)

!     ----------EQUIVALENTE STRAIN CORRECTOR FOR CONFINEMENT ----------
call calc_confinement_coef(3,psije,psijebn,coef_eeq)

!     -----------------TENSION AND COMPRESSION STRAIN -----------------
call calc_signal_strain(3,E,nu,eijt,psije,ONE)
call calc_signal_strain(3,E,nu,eijc,psije,mONE)
  
!     ------POSITIVE TENSION AND COMPRESSION STRAIN WITH BRACKET -------
call calc_macaulay_bracket(3,eijt,eijtbp,ONE)
call calc_macaulay_bracket(3,eijc,eijcbp,ONE)
  
!     ----------------TENSION AND COMPRESSION PARAMETERS----------------
call calc_alpha_e(eijtbp,eijcbp,3,alphat,alphac)
...
!     --------------UPDATED STRESS AND EFFECTIVE STRESS-----------------
...
do i=1,3
	if (signal>=ZERO) then
		mat_b(i)=max(ZERO,mat(i))
	else
		mat_b(i)=min(ZERO,mat(i))
	end if
end do
...
if (Gf<=ZERO) then
  ...
  ! if (GfIt<=ZERO) then
    ! dt=ONE-ed0*(ONE-At)/eeq-At/exp(Bt*(eeq-ed0))
  ! else
    ! et1=max(0.15d-3,1.1d0*ed0)                                    ! STRAIN BEFORE THE BEGGING OF SOFTENING
    ! etu=GfIt/(celent*1.111d0*E*ed0)+et1                           ! STRAIN FOR LINEAR SOFTENING
    ! etp1=et1-(etu-et1)*log(pt)                                    ! STRAIN AT RESIDUAL STRESS
    ! if (eeq>ed0 .and. eeq<=et1) then
      ! const1=(1.111d0*E*ed0)*(ONE-0.1d0*(et1-eeq)/(etu-et1))      ! STRESS AFTER 0.9*fctm
      ! dt=ONE-const1/(E*eeq)
    ! else if (eeq>et1 .and. eeq<=etp1) then
      ! dt=ONE-1.111d0*ed0/eeq*exp((et1-eeq)/(etu-et1))
    ! else
      ! dt=ONE-pt*et1/eeq
    ! end if
  ! end if
...
coef=ONE
if ((psijbn(1)+psijbn(2)+psijbn(3))<ZERO) then
  const1=ZERO
  const2=ZERO
  do i=1, 3
    const1=const1+(psijbn(i))**2
    const2=const2+psijbn(i)
  end do
...
subroutine calc_signal_strain(ndi,E,nu,peij,psij,signal)
...
!     ------------------------REAL VARIABLES----------------------------
real(kind=8), intent(in)  :: psij(3),E,nu,signal
real(kind=8), intent(inout) :: peij(3)
real(kind=8) :: psbij(3), trace_psij, trace_psbij

!     ------------DOUBLE PRECISION VALUES OF 0,1,2,3,1/2----------------
real(kind=8), parameter :: ZERO=0.d0, ONE=1.d0, TWO=2.d0, THREE=3.d0,&
                           FOUR=4.d0, HALF=0.5d0

trace_psij=psij(1)+psij(2)+psij(3)
call calc_macaulay_bracket_scalar(trace_psij,trace_psbij,signal)
call calc_macaulay_bracket(ndi,psij,psbij,signal)

peij(1)=(ONE+nu)/E*psbij(1)-nu/E*trace_psbij
peij(2)=(ONE+nu)/E*psbij(2)-nu/E*trace_psbij
peij(3)=(ONE+nu)/E*psbij(3)-nu/E*trace_psbij
